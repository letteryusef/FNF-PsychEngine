...
class PlayState extends MusicBeatState
{
	...
	public var camZoomingDecay:Float = 2.88;
	...
	override public function create()
	{
		...
		inverse = ClientPrefs.getGameplaySetting('inverse', false);
		...
		ouchUI = new FlxSprite(!inverse ? healthBarBG.x + 387 : healthBarBG.x + 4, healthBarBG.y + 4.5, Paths.image('ouch'));
		ouchUI.scale.y = 1.08;
		ouchUI.alpha = 0;
		ouchUI.angle = !inverse ? 0 : 180;
	...
	public function updateScore(miss:Bool = false)
	{
		if(ClientPrefs.scoreZoom && !miss && !cpuControlled)
		{
			...
			scoreTxtTween = FlxTween.tween(scoreTxt.scale, {x: 1, y: 1}, 0.6, {ease:FlxEase.quintOut,
				onComplete: function(twn:FlxTween) {
					scoreTxtTween = null;
				}
			});
	...
	function startSong():Void
	{
		...
		DiscordClient.changePresence(detailsText, SONG.song + " (" + storyDifficultyText + ")", "icon", true, songLength, imageKey);
		#end
	...
	private function generateSong(dataPath:String):Void
	{
		...
		for (section in noteData)
		{
			for (songNotes in section.sectionNotes)
			{
				...
				swagNote.mustPress = inverse ? !gottaHitNote : gottaHitNote;
				...
				if(floorSus > 0) {
					...
					for (susNote in 0...floorSus)
					{
						...
						sustainNote.mustPress = inverse ? !gottaHitNote : gottaHitNote;
	...
	private function generateStaticArrows(player:Int):Void
	{
		for (i in 0...4)
		{
			...
			if (player == 1)
			{
				if (!inverse) playerStrums.add(babyArrow) else opponentStrums.add(babyArrow);
			}
			else
			{
				...
				if (!inverse) opponentStrums.add(babyArrow) else playerStrums.add(babyArrow);
			}
	...
	override function closeSubState()
	{
		if (paused)
		{
			...
			if (startTimer != null && startTimer.finished)
			{
				DiscordClient.changePresence(detailsText, SONG.song + " (" + storyDifficultyText + ")", "icon", true, songLength - Conductor.songPosition - ClientPrefs.noteOffset, imageKey);
			}
			else
			{
				DiscordClient.changePresence(detailsText, SONG.song + " (" + storyDifficultyText + ")", "icon", null, null, imageKey);
			}
			#end
		}
	...
	override public function update(elapsed:Float)
	{
		...
		if (showNoteCombo)
		{
			...
			{
				lastMustHit = SONG.notes[curSection].mustHitSection;
				if (((!inverse && !SONG.notes[curSection].mustHitSection) || (inverse && SONG.notes[curSection].mustHitSection)) && !endingSong && !inCutscene && noteComboNumberlol > 5 && (curBeat % 4 == 0 || curBeat % 6 == 0))
				{
			...
			if (noteShit.animation.finished)
			{
		...
		if (allowHarmonyEffect && hasMoreAnimationsDAD)
		{
			...
			savedAnimationsDAD = [];
			hasMoreAnimationsDAD = false;
		}

		if (allowHarmonyEffect && hasMoreAnimationsBF)
		{
		...
		if (health > 0 && !paused && countdownFinished)
		{
	...
	function playAnimationAdvanced(object:FlxSprite, anim:String, offsetAddX:Float = 0, offsetAddY:Float = 0)
	{
		...
		object.animation.play(anim, true);
	}
	...
	function openPauseMenu()
	{
		...
		#if desktop
		DiscordClient.changePresence(detailsPausedText, SONG.song + " (" + storyDifficultyText + ")", "icon", null, null, imageKey);
		#end
	}
	...
	function doDeathCheck(?skipHealthCheck:Bool = false) {
		if ((!inverse && ((skipHealthCheck && instakillOnMiss) || health <= 0) && !practiceMode && !isDead) || (inverse && ((skipHealthCheck && instakillOnMiss) || health >= 2) && !practiceMode && !isDead))
		{
			...
			if(ret != FunkinLua.Function_Stop) {
				...
				DiscordClient.changePresence("Game Over - " + detailsText, SONG.song + " (" + storyDifficultyText + ")", "icon", null, null, imageKey);
				#end
				isDead = true;
				return true;
			}
		...
		switch(eventName) {
			...
			case 'Tutorial Subtitles':
				if (ClientPrefs.language != "English" || (ClientPrefs.language == 'English' && ClientPrefs.subtitles))
				{
					...
					var subtitles:Array<Dynamic> = Language.uiTexts.get('tutorialSubtitles');
					if (value2 == 'add')
					{
						setSubtitle(subtitles[val1], true);
					} else {
						setSubtitle(subtitles[val1]);
					}
				}
		}
	...
	function noteMiss(daNote:Note):Void { //You didn't hit the key and let it go offscreen, also used by Hurt Notes
		...
		var char:Character = !inverse ? boyfriend : dad;
		if(daNote.gfNote) {
			char = gf;
		}
	...
	function opponentNoteHit(note:Note):Void
	{
		...
		if (!note.isSustainNote)
		{
			...
			if (allowHarmonyEffect && !inverse && savedAnimationsDAD.length >= 2)
			{
				hasMoreAnimationsDAD = true;
			} else if (allowHarmonyEffect && inverse && savedAnimationsBF.length >= 2)
			{
	...
	function goodNoteHit(note:Note):Void
	{
		...
		if (!note.wasGoodHit)
		{
			...
			if (!inverse) health += note.hitHealth * healthGain else health -= note.hitHealth * healthGain;
			...
			if(!note.noAnimation) {
				...
				if (allowHarmonyEffect && !note.isSustainNote)
				{
					...
					{
						savedAnimationsDAD.insert(savedAnimationsDAD.length + 1, animToPlay + note.animSuffix);
					} else if (note.gfNote && gf != null)
					{
			...
			if (!note.isSustainNote)
			{
				...
				if (allowHarmonyEffect && !inverse && savedAnimationsBF.length >= 2)
				{
					hasMoreAnimationsBF = true;
				} else if (allowHarmonyEffect && inverse && savedAnimationsDAD.length >= 2)
				{
					hasMoreAnimationsDAD = true;
				} else if (allowHarmonyEffect && savedAnimationsGF.length >= 2)
				{
	...
	public var noteSplashSpawn = true;

	function spawnNoteSplashOnNote(note:Note) {
		if(ClientPrefs.noteSplashes && note != null && noteSplashSpawn) {
	...
	override function sectionHit()
	{
		...
		if (SONG.notes[curSection] != null)
		{
			...
			if (showNoteCombo != false && curSection > SONG.notes.length - 2)
			{
				showNoteCombo = false;
			}
