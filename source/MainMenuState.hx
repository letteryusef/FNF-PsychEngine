...
class MainMenuState extends MusicBeatState
{
	...
	override function create()
	{
		...
		for (i in 0...optionShit.length)
		{
			...
			var menuItem:FlxSprite = new FlxSprite(0, 20 + (i * 132)  + offset);
		...
		var versionShit:FlxText = new FlxText(12, FlxG.height - 44, 0, "Psych Engine v" + psychEngineVersion + " (WaveNami Ver.)", 12);
	...
	override function update(elapsed:Float)
	{
		...
		if (!selectedSomethin)
		{
			...
			if (FlxG.mouse.justPressed && mouseOn) // repeated cuz of the mouseOn
			{
				accept();
			}
			#if desktop
